...
func TestMap_OmitEmpty(t *testing.T) {
	type A struct {
		Name  string
		Value string    `shar:",omitempty"`
		Time  time.Time `shar:",omitempty"`
	}
	a := A{}
...
func TestMap_OmitNested(t *testing.T) {
	type A struct {
		Name  string
		Value string
		Time  time.Time `shar:",omitnested"`
	}
	a := A{Time: time.Now()}
...
func TestMap_NestedSliceWithStructValues(t *testing.T) {
	type address struct {
		Country string `shar:"customCountryName"`
	}

	type person struct {
		Name      string    `shar:"name"`
		Addresses []address `shar:"addresses"`
	}
...
func TestMap_InterfaceTypeWithMapValue(t *testing.T) {
	type A struct {
		Name    string      `shar:"name"`
		IP      string      `shar:"ip"`
		Query   string      `shar:"query"`
		Payload interface{} `shar:"payload"`
	}
