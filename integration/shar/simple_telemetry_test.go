...
func TestSimpleTelemetry(t *testing.T) {
	...
	d := &testSimpleTelemetryHandlerDef{t: t, finished: make(chan struct{})}

	err = taskutil.RegisterTaskYamlFile(ctx, cl, "simple_test.yaml", d.integrationSimpleTelemetry)
	require.NoError(t, err)
...
func (d *testSimpleTelemetryHandlerDef) integrationSimpleTelemetry(_ context.Context, _ client.JobClient, vars model.Vars) (model.Vars, error) {
	fmt.Println("Hi")
